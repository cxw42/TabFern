...
function ctor() {
    //([type string, ]key_names [, other_names])
    let ty_string;
    let key_names;
    let other_names;
    ...
    if (!(proto_key in Protos)) {
        // Need to make the instance functions

        /// Make a new value for storage.  Does not add the value
        /// to the multidex.
        function new_value() {
            return make_value(all_names, ty_string);
        }
        ...
        for (let key_name of key_names) {
            this_proto[ACC + key_name] = function (key_val, field_name = null) {
                ...
                if (field_name == null || typeof field_name === "undefined") {
                    return val;
