    ...
    $.jstree.core.prototype = {
        ...
        check : function (chk, obj, par, pos, more) {
            ...
            if(chk === "move_node" || chk === "copy_node") {
                ...
                // Extra check with custom error - hack pending the official fix
                // to vakata/jstree#2237
                if(chk === "move_node" && $.inArray(obj.id, par.children) === pos) {    // Nothing to do
                    this._data.core.last_error = { 'error' : 'check', 'plugin' : 'core', 'id' : 'tabfern_nop', 'reason' : 'Nothing to do', 'data' : JSON.stringify({ 'chk' : chk, 'pos' : pos, 'obj' : obj && obj.id ? obj.id : false, 'par' : par && par.id ? par.id : false }) };
                    return false;
                }

                if((!more || !more.is_multi) && (obj.id === par.id || $.inArray(par.id, obj.children_d) !== -1)) {
